...
| `HTTPMCPClient` | Cliente JSON-RPC HTTP real con token por entorno y respuesta acotada |
| Mock tools | Simulaciones de búsqueda, calendario y email para el MVP |
| `EngineEvent` | Contrato de eventos públicos e internos |
| `PersistenceRepository` | Round-trip de estado, sanitización, optimistic locking y audit trail |
| `AIBackendService` | Fachada autorizada que carga, ejecuta, persiste y devuelve DTOs |
| `UserBudgetManager` | Rate limit, coste estimado y tiempo máximo por usuario |
| `TelemetrySink` | Observabilidad de LLM, tools y eventos sin payload sensible |
...
- [mcp_http.py](<D:/Proyectos2026/agent-sync/backend/ai/tools/mcp_http.py>)
- [repository.py](<D:/Proyectos2026/agent-sync/backend/persistence/repository.py>)
- [dto.py](<D:/Proyectos2026/agent-sync/backend/ai/api/dto.py>)
- [service.py](<D:/Proyectos2026/agent-sync/backend/ai/service.py>)
...
- La Persistencia SQLModel ya tiene un adaptador compatible, pero todavía debe
  integrarse con las migraciones y el ciclo de vida transaccional de la rama
  `feat/data-model`.
- Backend API todavía debe exponer estos DTOs mediante endpoints HTTP/WebSocket
  con autenticación, CORS, paginación y versionado.
- Transporte debe convertir los `EngineEvent` en mensajes, notificaciones y eventos
  idempotentes.
- El cliente MCP HTTP real está definido, pero la autenticación OAuth, el catálogo
  de servidores y los servicios externos todavía deben configurarse por entorno.
- Las tools simuladas continúan siendo el proveedor por defecto del MVP.
- El rate/cost budget actual es local al proceso; un despliegue multi-worker debe
  reemplazarlo por un ledger distribuido.
